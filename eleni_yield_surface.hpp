...
#if !defined(KRATOS_ELENI_YIELD_SURFACE_H_INCLUDED )
#define      KRATOS_ELENI_YIELD_SURFACE_H_INCLUDED
...
namespace Kratos
{
  ...
  class KRATOS_API(CONSTITUTIVE_MODELS_APPLICATION) EleniYieldSurface : public YieldSurface<THardeningRule>
  {    
  public:
    ...
    /// Pointer definition of EleniYieldSurface
    KRATOS_CLASS_POINTER_DEFINITION( EleniYieldSurface );
    ...
    /// Default constructor.
    EleniYieldSurface() : BaseType() {}

    /// Copy constructor.
    EleniYieldSurface(EleniYieldSurface const& rOther) : BaseType(rOther) {}


    /// Assignment operator.
    EleniYieldSurface& operator=(EleniYieldSurface const& rOther)
    {
    ...
    {
      return BaseTypePointer(new EleniYieldSurface(*this));
    }

    /// Destructor.
    virtual ~EleniYieldSurface() {}
    ...
    virtual double& CalculateYieldCondition(const PlasticDataType& rVariables, double & rYieldCondition) override
    {
	  ...
	  const double& rBeta = rMaterialProperties[BETA];
	  const double& rAa = rMaterialProperties[AA];
	  ...
	  double rHp; //Irene 22/10
	  
      rHp = (- (1.0 / rBeta) * (pow((MeanStress) / (MeanStress - rPCstar) - rAa,2)));
	  rHp = exp(rHp);
      rYieldCondition  = pow( ThirdInvariantEffect * DeviatoricQ/rShearM, 2);
      rYieldCondition += (rHp * (MeanStress - 2.0 * rPT) * (MeanStress - rPCstar) );
    ...
    virtual VectorType& CalculateDeltaStressYieldCondition(const PlasticDataType& rVariables, VectorType& rDeltaStressYieldCondition) override
    {
	 ...
	 double rDeltaStressHp;

      rDeltaStressHp = rHp * (- (2.0 / rBeta) * ((- rPCstar) / pow(MeanStress - rPCstar,2)));

      rDeltaStressYieldCondition = (rDeltaStressHp * (MeanStress - 2.0 * rPT) * (MeanStress - rPCstar) + rHp * (MeanStress - rPCstar) + rHp * (MeanStress - 2.0 * rPT)) * V1;
      rDeltaStressYieldCondition += 2.0 * 3.0 * pow( ThirdInvariantEffect / rShearM, 2) * J2 * V2;
    ...
    {
      rOStream << "EleniYieldSurface";
    }

    /// Print object's data.
    virtual void PrintData(std::ostream& rOStream) const override
    {
      rOStream << "EleniYieldSurface Data";
    }
...
#endif // KRATOS_ELENI_YIELD_SURFACE_H_INCLUDED  defined 
